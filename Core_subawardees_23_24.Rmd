...
#version = "20250606T145440Z-04a0e"
...
# lookup_cultspec <- read_excel(file.path(data_folder,
#                                         "Quantitative",
#                                         "Lookup Tables",
#                                         "Data_Element_Crosswalk_state_Core_04102024.xlsx"))
...
## Data Version Used:
# Note: 06/04 data was actually pulled on 04/28
...
plot_shelter_type <- ggplot(regional_type_plot, 
  ...
  geom_text(aes(label=comma(totals)), vjust=-0.3, size=3.5, position = position_dodge(width = 0.9)) +
...
awards are usually higher if a smaller number given out. 
...
plot_shelter_region <- ggplot(regional_type2, 
                            aes(x = Region, y = `Total Subawardees`, 
                                fill = Fy)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_text(aes(label=comma(`Total Subawardees`)), vjust=-0.5, size=3.5, position = position_dodge(width = 0.9)) + 
  scale_fill_manual(values = c("FY23" = "#BCD9ED", "FY24" = "#264A64")) +
...
plot_nonshelter_region <- ggplot(regional_type3, 
                            aes(x = Region, y = `Total Subawardees`, 
                                fill = Fy)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_text(aes(label=comma(`Total Subawardees`)), vjust=-0.3, size=3.5, position = position_dodge(width = 0.9)) +
